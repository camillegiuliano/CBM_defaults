...
[CBM_defaults](https://github.com/PredictiveEcology/CBM_defaults.git) provides simulations with default parameter values for the managed forests of Canada, akin to the Archive Index in the CBM-CFS3 documentation. Most parameters are read in from various tables from a SQLite database file (link below). Some defaults are modified from CBM-CFS3 to accommodate the capacity for a more flexible representation of vegetation dynamics enable by the `libcbm` Python functions. There are available [here](https://github.com/cat-cfs/libcbm_py/tree/main/libcbm/resources/cbm_exn). Defaults for the managed forests of Canada include disturbance data such as different disturbance types and their IDs, spinup parameters, pool names, and more. The outputs of this module are primarily used [CBM_dataPrep_SK](https://github.com/PredictiveEcology/CBM_dataPrep_SK.git) and [CBM_core](https://github.com/PredictiveEcology/CBM_core.git).
